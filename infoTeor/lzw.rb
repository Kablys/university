...
case mode
when 'en'
  #todo add blocking argument
  size, input_path, output_path = rest
  input_file = File.open(input_path, 'rb') { |io| io.read }
  compressed_string = LZW::Compressor.new(max_code_size: size.to_i).compress(input_file)
  File.open(output_path, 'wb') { |io| io.write(compressed_string) }
when 'de'
  input_path, output_path  = rest
  input_file = File.open(input_path, 'rb') { |io| io.read }
  decompressed_string = LZW::Decompressor.new.decompress(input_file)
  File.open(output_path, 'wb') { |io| io.write(decompressed_string) }
else
  puts "Wrong arguments, should be:"
  puts 'ruby main.rb en size input_path output_path'
  puts 'ruby main.rb de input_path output_path'
end
