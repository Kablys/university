...
--lazyGet :: IO [Word16]
lazyGet file steps size remainder = do
...
-- TODO get Rem
encodeFile file size = do
  fileSize <-  withFile file ReadMode hFileSize
  let --size = 2
  ...
  (source, rm) <- lazyGet file steps size remainder
...
abcIndex size = [2^size-1,2^size-2..0]

test = do
  let abcIndex sizeI = [2^sizeI-1,2^sizeI-2..0]
  (file, head) <- encodeFile "100kAs.txt" 2
  let size = 2
...
main = do
  ...
  case arg of
    ...
    ("en":size:from:to:[]) -> do
      print "encoding"
      let s = read size :: Int
      (file, header) <- encodeFile from s
      let encoded = map elias $ encode (abcIndex s) file
          --el =  map elias encoded
      out <- elToFile (concat encoded)
      BL.writeFile to $ BL.append header out
      --conntent <- BL.read
      --putStrLn conntent
    ("de":from:to:[])-> do
      file <- BS.readFile from
      (enFile, head2) <- elFromFile file  --(BL.toStrict outHead)
      let hSize = fromIntegral (getSize head2) :: Int
          hRSize = fromIntegral (getRemSize head2) :: Int
          hRem = if hRSize == 0 then BL.empty else BP.runBitPut (BP.putNBits (hRSize) (getRem head2))
      --traceM("encoded file " ++ show enFile)
    ...
    _ -> error $ (show arg) ++ "\nWrong arguments: modInt en SIZE(2-16) SOURCE DEST"
